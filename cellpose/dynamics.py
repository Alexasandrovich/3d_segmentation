...
def masks_to_flows_custom(mask):
    """ convert masks to flows using distance-transform
     ...
     """
    # mu_c = cv2.distanceTransform(np.array(mask), cv2.DIST_L2, 3)

    Ly, Lx = mask.shape
    mu = np.zeros((2, Ly, Lx), np.float64)
    horizontal_gradients = np.gradient(mask, axis=1)
    vertical_gradients = np.gradient(mask, axis=0)
    mu[0] = vertical_gradients
    mu[1] = horizontal_gradients
    return mu, None


def masks_to_flows(masks, use_gpu=False, device=None, custom_collecting=True):
    """ convert masks to flows using diffusion from center pixel
    ...
    custom_collecting:
        collect flows not for medical data with a lot of centroids
    masks: int, 2D or 3D array
        labelled masks 0=NO masks; 1,2,...=mask labels
    ...
    if masks.ndim==3:
        ...
        if not custom_collecting:
            mu, mu_c = masks_to_flows_device(masks, device=device)
        else:
            mu, mu_c = masks_to_flows_custom(masks)
        return mu
