...
from cellpose import utils, plot, transforms
...
def get_image_files(folder, mask_filter, imf=None, look_one_level_down=False):
    ...
    for im in image_names:
        ...
        igood = all([(len(imfile) > len(mask_filter) and imfile[-len(mask_filter):] != mask_filter) or len(
            imfile) <= len(mask_filter)
...
def get_label_files(image_names, mask_filter, imf=None):
    ...
    elif os.path.exists(label_names0[0] + '_flows.png'):
        flow_names = [label_names0[n] + '_flows.png' for n in range(nimg)]
    else:
    ...
    if os.path.exists(label_names[0] + mask_filter + '.json'):  # todo: add preprocesing as is
        label_names = [label_names[n] + mask_filter + '.tif' for n in range(nimg)]
    elif os.path.exists(label_names[0] + mask_filter + '.tif'):
        label_names = [label_names[n] + mask_filter + '.tif' for n in range(nimg)]
...
def load_train_test_data(train_dir, test_dir=None, image_filter=None, mask_filter='_masks', unet=False,
    ...
    if test_dir is not None:
        test_images, test_labels, test_image_names = load_images_labels(test_dir, mask_filter, image_filter,
                                                                        look_one_level_down, unet)
