...
from . import utils, io_custom, transforms
...
def show_segmentation(img, maski, flowi, label, channels=[0,0], file_name=None):
    ...
    _, maski = cv2.threshold(maski, 0, 255, cv2.THRESH_BINARY)
    outlines = utils.masks_to_outlines(maski)

    overlay = mask_overlay(img0, maski)
    outX, outY = np.nonzero(outlines)
    imgout= img0.copy()
    imgout[outX, outY] = np.array([25,255,25]) # pure red

    dP = np.zeros((2, img0.shape[0], img0.shape[1]))
    dP[0] = label[2]
    dP[1] = label[3]
    ideal_as_gradients = dx_to_circ(dP)
    ...
    axes[1].imshow(ROI(ideal_as_gradients, roi))
    axes[1].set_title('ideal flows', loc='right', backgroundcolor='white')
    axes[1].axis('off')

    cv2.imwrite('predicted_flows.png', flowi)
    cv2.imwrite('ideal_flows.png', ideal_as_gradients)
