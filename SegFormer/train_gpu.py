...
from utils.schedulers import create_lr_scheduler
...
def get_argparser():
    ...
    # Datset Options
    parser.add_argument("--data_root", type=str, default='./datasets/data/scans/', help="path to Dataset")
    parser.add_argument("--image_size", type=int, default=[2048, 1024], help="input size")
    parser.add_argument("--ignore_label", type=int, default=100, help="path to Dataset")
    parser.add_argument("--dataset", type=str, default='scans', choices=['scans'], help='Name of dataset')
    ...
    parser.add_argument("--amp", type=bool, default=True,
                        help='auto mixture precision')  # There may be some problems when loading weights, such as: ComplexFloat
...
def main(args):
    ...
    cur_log_dir = Path(args.log_dir).joinpath(f"inference_res_{date}")
    cur_log_dir.mkdir(exist_ok=True, parents=True)
    results_file = cur_log_dir.joinpath("results.txt")
    ...
    train_set = Scans(args.data_root, 'train', train_transform)
    valid_set = Scans(args.data_root, 'test', val_transform)
    ...
    for epoch in range(args.epochs):
        ...
        confmat = evaluate(args, model, valloader, device, args.val_print_freq, valid_set, epoch, cur_log_dir)
