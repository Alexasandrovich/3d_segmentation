...
class MiT(nn.Module):
    ...
    def forward(self, x, return_layer_outputs=False):
        ...
        for (get_overlap_patches, overlap_embed, layers) in self.stages:
            ...
            new_h, new_w = self.calculate_new_dimensions(original_h, original_w, num_patches, False)

            x = rearrange(x, 'b c (h w) -> b c h w', h=new_h, w=new_w)
    ...
    def calculate_new_dimensions(self, original_h, original_w, num_patches, need_logs=False):
        for ratio in range(1, max(original_h, original_w) + 1):
            ...
            if new_h * new_w == num_patches:
                ...
                if need_logs:
                    print(f"{new_h} * {new_w} = {new_h * new_w} != {num_patches}")
        raise ValueError("Failed to find the appropriate dimensions for the new dimensions.")
