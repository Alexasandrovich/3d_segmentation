...
def train_model(root, model_type='cellpose', cell_type=None, tissue_type=None, 
                platform_type=None, full_img=False,
                ntrain=10, ntest=0, weight_decay=1e-4, n_epochs=250, learning_rate = 0.1, batch_size=8,
                nimg_per_epoch=8, start_pretrained = False, run_eval = True, save_results=True,
                save_test_masks =False, seed=1, flow_threshold=0.4,
                pretrained_model=None):
    
    if model_type=='cellpose':
        netstr = 'cytotorch_0' if start_pretrained else 'scratch'
        if pretrained_model is not None:
            pretrained_model = os.fspath(models.MODEL_DIR.joinpath(netstr)) if start_pretrained else None
    ...
    k=0
    while nmasks==0:
        train_files, channels, netstrf = get_train_files(root, cell_type, tissue_type, platform_type, 
                                                        ntrain=ntrain,
                                                        full_img=full_img, seed=seed)
        train_data, train_masks = load_data_masks(train_files, frac=ntrain if ntrain<1 else 1.0)
        ...
        if nmasks==0:
            ...
            k+=1
    netstr += netstrf
    print(f'seed = {seed}, nmasks = {nmasks}, train_files[0] = {os.path.split(train_files[0])[-1]}')
    ...
    model_path = model.train(train_data, train_masks, train_files=None, 
                              ...
                              save_path=root, save_every=n_epochs,
    ...
    if run_eval:
        ...
        diam_train = np.array([utils.diameters(train_masks[k])[0] for k in range(len(train_masks))])
        diam_train[diam_train<5] = 5.
        diam_mean = diam_train.mean()
        
        eval_model(model_path, diam_mean, nmasks, root, model_type=model_type,
                    cell_type=cell_type, tissue_type=tissue_type, platform_type=platform_type, 
                   ntest=ntest, save_results=save_results, save_test_masks=save_test_masks,
...
def eval_model(model_path, diam_mean, nmasks, root, model_type='cellpose',
               cell_type=None, tissue_type=None, platform_type=None, 
               ntest=0, flow_threshold=0.4,
               save_results=False, save_test_masks=False):
    ...
    if save_results:
        np.save(os.path.join(root, f'models/{netstr}_AP_TP_FP_FN.npy'), {'threshold':threshold, 'ap':ap,
                                                                         'tp':tp, 'fp':fp, 'fn':fn, 
                                                                         'ntrain_masks':nmasks, 
...
def eval_model_cyto(root, model_type='cellpose',
               cell_type=None, tissue_type=None, platform_type=None, 
               ntest=0, flow_threshold=0.4,
               save_results=False, save_test_masks=False):
    if model_type=='cellpose':
    ...
    if len(train_files) > 0:
        ...
        eval_model(model_path, diam_mean, 0, root, model_type=model_type,
                ...
                platform_type=platform_type, save_results=save_results, flow_threshold=flow_threshold)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='training parameters')
        
    # settings for locating and formatting images
    parser.add_argument('--dir', default=[], type=str, help='folder containing data to train and test on.')
    ...
    parser.add_argument('--model_type', default='cellpose', type=str, help=' model type (cellpose or unet)')
    parser.add_argument('--cell_type', default=None, type=str, help='livecell cell_type')
    parser.add_argument('--tissue_type', default=None, type=str, help='tissuenet tissue_type')
    parser.add_argument('--platform_type', default=None, type=str, help='tissuenet platform_type')
    parser.add_argument('--seed', default=1, type=int, help='random seed for train set')
    parser.add_argument('--ntrain', default=10, type=float, help='# of images in train set, if 0 uses all images')
    parser.add_argument('--ntest', default=50, type=int, help='# of images in test set')
    parser.add_argument('--flow_threshold', default=0.4, type=float, help='flow_threshold')
    ...
    if args.train:
        train_model(args.dir, model_type=args.model_type, cell_type=args.cell_type, tissue_type=args.tissue_type, 
                    platform_type=args.platform_type, start_pretrained=args.start_pretrained, seed=args.seed, 
